classdef HW2_Utils
    ...
    methods (Static)
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        function [D, lb, imRegs] = getNegex(w, b, dataset)
            load(sprintf('%s/%sAnno.mat', HW2_Utils.dataDir, dataset), 'ubAnno');
            ...
            for i=1:length(ubAnno)
                ...
                [imH, imW,~] = size(im);
                negRects = HW2_Utils.detect(im, w, b);
                randRects = negRects(:, negRects(5,:) < 0);
                ...
                nNeg2SamplePerIm = min(20, size(randRects, 2));
                [D_i, R_i] = deal(cell(1, nNeg2SamplePerIm));
                for j=1:nNeg2SamplePerIm
                    imReg = im(int16(randRects(2,j)):int16(randRects(4,j)), int16(randRects(1,j)):int16(randRects(3,j)),:);
                    imReg = imresize(imReg, HW2_Utils.normImSz);
                    R_i{j} = imReg;
                    D_i{j} = HW2_Utils.cmpFeat(rgb2gray(imReg));                    
                end
        ...
        function [featVecarray] = createNegexamples2(dataset, w, b)
            load(sprintf('%s/%sAnno.mat', HW2_Utils.dataDir, dataset), 'ubAnno');
            %[posD, negD, posRegs, negRegs] = deal(cell(1, length(ubAnno)));
            featVecarray = zeros(1984, 1); % hard coded
            cnt = 1;
            for i = 1:length(ubAnno)
                ml_progressBar(i, length(ubAnno), 'Processing image');
                im = imread(sprintf('%s/%sIms/%04d.jpg', HW2_Utils.dataDir, dataset, i));
                rects = HW2_Utils.detect(im, w, b);
                % sort(rects(5, :));
                neg_rects = zeros(5, 1);
                k = 1;
                for ab = 1:size(rects, 2)
                    if rects(5, ab) < 0
                        neg_rects(:, k) = rects(:, ab);
                        k = k + 1;
                    end
                end
                ubs = ubAnno{i}; % annotated upper body
                for j=1:size(ubs,2)
                    overlap = HW2_Utils.rectOverlap(neg_rects, ubs(:,j));                    
                    neg_rects = neg_rects(:, overlap < 0.3);
                    if isempty(neg_rects)
                        break
                    end
                end
                
                % Compute HOG feature for first 10 neg_rects
                % change this part if you need more than 10 negetive
                % examples
                [imH, imW,~] = size(im);
                for bc = 1:10
                    if neg_rects(3,bc) < imW && neg_rects(4,bc) < imH
                        % get the patch of negative Image
                        imReg = im(neg_rects(2,bc):neg_rects(4,bc), neg_rects(1,bc):neg_rects(3,bc), :);
                        imReg = imresize(imReg, HW2_Utils.normImSz);
                        featVec = vl_hog(im2single(rgb2gray(imReg)), HW2_Utils.hogCellSz);            
                        featVec = featVec(:);
                        featVecarray = featVec;
                        cnt = cnt + 1;
                    end
                end
            end
        ...
        function [negaD, negaLb, negaRegs] = createNegexamples(dataset)
            load(sprintf('%s/%sAnno.mat', HW2_Utils.dataDir, dataset), 'ubAnno');
            [posD, negD, posRegs, negRegs] = deal(cell(1, length(ubAnno)));            
            ...
            negD = cat(2, negD{:});   
            negaD = cat(2, negD);
            negaLb = [-ones(size(negD,2), 1)];
            negaRegs = cat(4, negRegs{:});            
        end
