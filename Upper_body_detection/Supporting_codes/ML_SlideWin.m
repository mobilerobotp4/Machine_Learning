...
% Implement efficient sliding window     
    ...
    methods
        ...
        % if nSample = [] or not defined, return all
        % if nSample > 0, return nSample random samples
        ...
        function [D, startLocs] = getBatch(self, batchId, nSample)
            startLocs = self.batchSubIdxs{batchId};            
            if exist('nSample', 'var') && nSample > 0 && nSample < size(startLocs,2)               
                idxs = randsample(length(self.batchLinIdxs{batchId}), nSample);
                startLocs = startLocs(:, idxs);
                A = repmat(self.winLinIdxs, 1, nSample) + ...
                    repmat(self.batchLinIdxs{batchId}(idxs), length(self.winLinIdxs), 1);                
            else                
        ...
        % Randomly sample some windows 
        % nSample: number of windows to sample
        % randWins: d*nSample, d: number of pixels in a window
        % randLocs: k*nSample matrix, k: number of diemnions of tensor data
        %   randLocs(i,j) is the pixel index the top-left for the i dimension of the j^th window
        function [randWins, randLocs] = getRandomSamples(self, nSample)
            nSamplePerBatch = ceil(nSample/self.nBatch);                        
    ...
    methods (Static)
        ...
        function demo2()
            ...
            nSample = 20;            
            [randWins, randLocs] = swObj.getRandomSamples(nSample);
